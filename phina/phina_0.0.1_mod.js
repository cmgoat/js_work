...
phina.namespace(function() {
	...
	phina.define('phina.display.CanvasElement', {
		...
		/**
		 * コルーチンセット
		 * task( ジェネレータ関数 ) > tasklistに指定したジェネレータ関数を格納
		 * taskrun() > update等から呼び出すことでtasklist内のジェネレータを全て進行させる。終了済みは自動的に取り除かれる。
		 * wait() > 指定回数分yield。yield* wait(n); の形で使う
		 */
		task:function(f){this.tasklist[this.tasklist.length]=f;},
		taskrun:function(){for(var i=0;i<this.tasklist.length;i++){if(this.tasklist[i].next().done){this.tasklist.splice(i,1);i--;}}},
...
phina.namespace(function() {
	...
	phina.define('phina.display.CanvasScene', {
		...
		ActorLayer_setup:function(num){	//num : レイヤー枚数指定
			...
			//レイヤーの親。これが動けば配下が全部動くのでスクロールやズーム表現が手軽に行える(多分)
			this.ActorLayerBase = phina.display.CanvasElement().addChildTo(this);

			this.ActorLayer=[];					//レイヤエリア作成
			this.ActorLayer_Property=[];			//レイヤソート情報
			this.ActorLayer_Num=num;				//レイヤ枚数

			//レイヤ作成 (大きい番号のレイヤ程奥になる)
			for(var i=this.ActorLayer_Num-1;i>-1;i--){
				this.ActorLayer_Property[i] = {};
				this.ActorLayer_Property[i].EnableSort = false;
				this.ActorLayer[i] = phina.display.CanvasElement().addChildTo(this.ActorLayerBase);
			}
		},

		//全アクターレイヤー内要素ソート
		ActorLayer_sort:function(){
			for(var i=0;i<this.ActorLayer_Property.length;i++){
				//ソート対象レイヤであるか判別
				if(this.ActorLayer_Property[i].EnableSort){
					//レイヤ内子要素全てにIDを割り振って安定ソート化する
					for(var j in this.ActorLayer[i].children){this.ActorLayer[i].children[j].sort_id=j;}
					//レイヤー毎のルールに従いソート実行
					this.ActorLayer[i].children.sort( this.ActorLayer_Property[i].SortRule );
				}
			}
		},

		//UIレイヤー機能準備
		UiLayer_setup:function(num){	//num : レイヤー枚数指定
			//レイヤーの親。これが動けば配下が全部動くのでスクロールやズーム表現が手軽に行える(多分)
			this.UiLayerBase = phina.display.CanvasElement().addChildTo(this);

			this.UiLayer=[];					//レイヤエリア作成
			this.UiLayer_Property=[];			//レイヤソート情報
			this.UiLayer_Num=num;				//レイヤ枚数

			//レイヤ作成 (大きい番号のレイヤ程奥になる)
			for(var i=this.UiLayer_Num-1;i>-1;i--){
				this.UiLayer_Property[i] = {};
				this.UiLayer_Property[i].EnableSort = false;
				this.UiLayer[i] = phina.display.CanvasElement().addChildTo(this.UiLayerBase);
			}
		},

		//全UIレイヤー内要素ソート
		UiLayer_sort:function(){
			for(var i=0;i<this.UiLayer_Property.length;i++){
				//ソート対象レイヤであるか判別
				if(this.UiLayer_Property[i].EnableSort){
					//レイヤ内子要素全てにIDを割り振って安定ソート化する
					for(var j in this.UiLayer[i].children){this.UiLayer[i].children[j].sort_id=j;}
					//レイヤー毎のルールに従いソート実行
					this.UiLayer[i].children.sort( this.UiLayer_Property[i].SortRule );
				}
			}
		},
...
phina.namespace(function() {
	...
	phina.define('phina.game.LoadingScene', {
		...
		update:function(){
			this.v+=3;
			this.C_Obj(this.width/2+Math.cos(this.v*Math.DEG_TO_RAD)*100,this.height/2+(Math.sin(this.v*Math.DEG_TO_RAD)*100+Math.cos(this.v*Math.DEG_TO_RAD)*100)/3,16,"rgba(20,20,96,1)").addChildTo(this);
			this.v+=180;
			this.C_Obj(this.width/2+Math.cos(this.v*Math.DEG_TO_RAD)*100,this.height/2+(Math.sin(this.v*Math.DEG_TO_RAD)*100+Math.cos(this.v*Math.DEG_TO_RAD)*100)/3,16,"rgba(96,20,20,1)").addChildTo(this);
			this.v+=180;


		},
		C_Obj:function(x,y,r,col){
			var c= phina.display.CircleShape();
			c.x=x;
			c.y=y;
			c.radius=r;
			c.fill=col;
			...
			c.blendMode = 'lighter';
			c.v=180;
			c.update = function(){c.radius-=0.4;if(c.radius<0){c.remove();}}
			return c;
		}
	});
});
...
//**************************************************
//追加 (tmlibからオミットされたっぽいカラーマトリックスフィルタ)
	/**
	 * @class phina.display.CanvasElement
	 * 参考
	 * http://rakuyudo.com/tmlib-js-sample80/
	 * http://hakuhin.jp/as/filter.html#COLOR_MATRIX_FILTER
	 */
/*　実装保留
phina.namespace(function() {
	phina.define('phina.graphics.ColorMatrixFilter', {
        ...
        apply: function(input, output) {
            ...
            for (var i=0,len=input.length; i<len; ++i) {
                var pixel = input.getPixelIndex(i);
                ...
                output.setPixel32Index(i, r, g, b, a);
            }
            return output;
        }
	});
...
*/
//**************************************************
//追加＆改変 (tmlibからオミットされたっぽいカラーマトリックスフィルタ)
...
/*
phina.namespace(function() {
	phina.define('phina.graphics.ColorNameFilter', {
        ...
        init: function(str) {
			...
			//入力した色名でカラーマトリックスを決める。該当なかったら最後のカラーマトリックスが適用される(すべての項目が1乗算なので何も起きない)
			for(var i=0;i<this.colorNameList.length;i++){
				if(str==this.colorNameList[i]||i==this.colorNameList.length){
					this.colorMatrix = colorlist[i];
				}
			}
